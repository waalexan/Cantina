...
if(isset($_COOKIE["clientecode"])){
  $_SESSION['clientecode']=$_COOKIE["clientecode"];
} else{
 // Setting a cookie
 setcookie("clientecode", random_int(1000000,8000000), time()+30*24*60*60);
}
$_SESSION['clientecode']=$_COOKIE["clientecode"];
$id;
...
<head>
  ...
  <title><?=$_COOKIE["clientecode"]?></title>
</head>
      ...
      <div class="corpo">
        ...
        <section style="width:95%">
          <?php if (isset($_REQUEST['code'])) : ?>
            <form action="./add.php" method="post" class="insert">
              <div>
          ...
          <div class="tabela">
            <table>
              ...
              <?php
              require 'config.php';
              $rows = mysqli_query($conn, "SELECT * FROM venda WHERE id_carrinho = '".$_SESSION['clientecode']."' ORDER BY `venda`.`id_cliente` DESC");
              $i = 1;
              $total;
              ?>
              <?php foreach ($rows as $row) : ?>
                <tr id=<?php echo $row["id"]; ?>>
                  ...
                  <?php $total += $row["total"]; ?>
                  <td>
                    <a id="delete" href="deletar.php?code=<?php echo $row['codigo']; ?>&qtd=<?php echo $row['qtd']; ?>&id=<?php echo $row['id_cliente']; ?>"><i class="bx bx-trash"></i> Cancelar</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            </table>
